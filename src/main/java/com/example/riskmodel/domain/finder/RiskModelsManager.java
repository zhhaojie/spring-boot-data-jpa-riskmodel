...
import com.example.riskmodel.domain.RiskModel;
...
public class RiskModelsManager {
    ...
    public static List<Class<?>> getModels() {
        try {
            return getClasses(PACKAGE_NAME).stream().filter(clazz -> clazz.isAnnotationPresent(RiskModel.class)).collect(Collectors.toList());
